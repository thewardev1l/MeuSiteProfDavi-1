// filmes.js

export const filmes = [
    {
        id: "interestelar",
        titulo: "Interestelar",
        imagem: "Imagens/interstellar.jpg",
        descricao: "Um grupo de astronautas viaja através de um buraco de minhoca em busca de um novo lar para a humanidade.",
        trailer: "i6avfCqKcQo?si=c67bqEGle7cgphln",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 49min",
        genero: "Ficção científica e aventura",
        classificacao: "14",
    },

    {
        id: "Capitão América: Guerra Civil",
        titulo: "Capitão América: Guerra Civil",
        imagem: "Imagens/capitaoamericaguerracivil.jpg",
        descricao: "Em Capitão América: Guerra Civil, danos colaterais em missões levam os governos mundiais a exigir o Tratado de Sokóvia, um sistema de controle e supervisão dos Vingadores. Isso divide a equipe: Steve Rogers rejeita a interferência política, enquanto Tony Stark apoia a regulamentação, gerando um conflito ideológico e físico entre os heróis.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 28min",
        genero: "Ação",
        classificacao: "12",
    },

    {
        id: "Homem de Ferro",
        titulo: "Homem de Ferro",
        imagem: "Imagens/homemdeferro.jpg",
        descricao: "O bilionário e inventor Tony Stark é sequestrado por terroristas no Afeganistão. Para escapar, ele constrói uma armadura de alta tecnologia. De volta aos EUA, decide parar de fabricar armas, melhora a armadura e assume a identidade do Homem de Ferro para derrotar seu sócio traidor e salvar o mundo.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 6min",
        genero: "Ação",
        classificacao: "12",
    },

    {
        id: "Jogos Vorazes",
        titulo: "Jogos Vorazes",
        imagem: "Imagens/vorazes.jpg",
        descricao: "Jogos Vorazes se passa em uma nação futurista chamada Panem, onde um governo cruel e totalitário comanda doze distritos empobrecidos a partir da poderosa Capital.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 22min",
        genero: "Ação/Terror",
        classificacao: "14",
    },

    {
        id: "Homem Aranha",
        titulo: "Homem Aranha: Um Novo Dia",
        imagem: "Imagens/homemaranhanovodia.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "16",
    },

    {
        id: "It",
        titulo: "It A Coisa",
        imagem: "Imagens/itacoisa.jpg",
        descricao: "Crianças começam a desaparecer de forma misteriosa na cidade de Derry, no estado do Maine, e um grupo de sete adolescentes desajustados que sofrem bullying, conhecido como o 'Clube dos Perdedores', se une para enfrentar uma criatura maligna milenar.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 15min",
        genero: "Terror",
        classificacao: "16",
    },

    {
        id: "FNAF",
        titulo: "Five Nights at Freddy's: O Pesadelo Sem Fim",
        imagem: "Imagens/fnaf.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Terror",
        classificacao: "14",
    },

    {
        id: "Pokemon",
        titulo: "Pokémon Detetive Pikachu",
        imagem: "Imagens/pokemon.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "12",
    },

    {
        id: "Jumanji",
        titulo: "Jumanji",
        imagem: "Imagens/jumanji.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "14",
    },

    {
        id: "Reze",
        titulo: "Chainsaw Man: Arco da Reze",
        imagem: "Imagens/reze.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "18",
    },

    {
        id: "fnaf2",
        titulo: "Five Nights at Freddy's 2",
        imagem: "Imagens/fnaf2.jpg",
        descricao: "",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Terror",
        classificacao: "14",
    },

    {
        id: "Gente Grande",
        titulo: "Gente Grande",
        imagem: "Imagens/gentegrande.jpg",
        descricao: "",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "12",
    },

    {
        id: "It Capítulo 2",
        titulo: "It Capítulo 2",
        imagem: "Imagens/it2.jpg",
        descricao: "",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "16",
    },

    {
        id: "H",
        titulo: "Homem Aranha",
        imagem: "Imagens/homemaranhanovodia.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "16",
    },

    {
        id: "Homem Aranha",
        titulo: "Homem Aranha",
        imagem: "Imagens/homemaranhanovodia.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "16",
    },

    {
        id: "Homem Aranha",
        titulo: "Homem Aranha",
        imagem: "Imagens/homemaranhanovodia.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "16",
    },

    {
        id: "Homem Aranha",
        titulo: "Homem Aranha",
        imagem: "Imagens/homemaranhanovodia.jpg",
        descricao: "O jovem Peter Parker que, após ser picado por uma aranha, ganha superpoderes como força, agilidade e a capacidade de escalar paredes.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "2h 24min",
        genero: "Ação",
        classificacao: "16",
    },

    {
        id: "senhor-dos-aneis",
        titulo: "O Senhor dos Anéis: A Sociedade do Anel",
        imagem: "Imagens/senhordosaneis.jpg",
        descricao: "O jovem Frodo recebe a missão de destruir um poderoso anel antes que ele caia nas mãos do Senhor das Trevas.",
        trailer: "ID_YOUTUBE",
        duracao: "2h 58min",
        genero: "Fantasia e aventura",
        classificacao: "12",
    },

    {
        id: "matrix",
        titulo: "Matrix",
        imagem: "Imagens/matrix.jpg",
        descricao: "Um programador descobre que a realidade em que vive é uma simulação criada por máquinas.",
        trailer: "ID_YOUTUBE",
        duracao: "2h 16min",
        genero: "Ficção científica e ação",
        classificacao: "14",
    },

    {
        id: "vingadores-ultimato",
        titulo: "Vingadores: Ultimato",
        imagem: "Imagens/vingadoresultimato.jpg",
        descricao: "Os Vingadores restantes tentam reverter as consequências do estalo de Thanos e salvar o universo.",
        trailer: "ID_YOUTUBE",
        duracao: "3h 1min",
        genero: "Ação e aventura",
        classificacao: "12",
    },

    {
        id: "batman",
        titulo: "The Batman",
        imagem: "Imagens/thebatman.jpg",
        descricao: "Batman investiga uma série de crimes em Gotham e descobre uma conspiração ligada às famílias mais poderosas da cidade.",
        trailer: "ID_YOUTUBE",
        duracao: "2h 56min",
        genero: "Ação, crime e suspense",
        classificacao: "14",
    },

    {
        id: "duna",
        titulo: "Duna",
        imagem: "Imagens/duna.jpg",
        descricao: "Paul Atreides viaja para um planeta desértico e se vê envolvido em uma disputa pelo controle de um recurso valioso.",
        trailer: "ID_YOUTUBE",
        duracao: "2h 35min",
        genero: "Ficção científica e aventura",
        classificacao: "14",
    },
];

export const series = [
    {
        id: "Stranger Things",
        titulo: "Stranger Things",
        imagem: "Imagens/strangerthings.jpg",
        descricao: "Um grupo de crianças enfrenta forças sobrenaturais.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "5 Temporadas",
        genero: "Suspense e ficção científica",
        classificacao: "16",
    },

    {
        id: "La Casa de Papel",
        titulo: "La Casa de Papel",
        imagem: "Imagens/lacasadepapel.jpg",
        descricao: "Um gênio do crime conhecido como O Professor recruta oito pessoas com habilidades específicas e sem nada a perder.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "5 Temporadas",
        genero: "Ação, crime, drama, policial e suspense",
        classificacao: "18",
    },

    {
        id: "Breaking Bad",
        titulo: "Breaking Bad",
        imagem: "Imagens/breakingbad.jpg",
        descricao: "A série acompanha Walter White e a sua transformação de um homem comum no chefão do tráfico.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "5 Temporadas",
        genero: "Drama, suspense e crime",
        classificacao: "18",
    },

    {
        id: "Peaky Blinders",
        titulo: "Peaky Blinders",
        imagem: "Imagens/peakyblinders.jpg",
        descricao: "Peaky Blinders acompanha Thomas Shelby e sua gangue em Birmingham após a Primeira Guerra Mundial.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "6 Temporadas",
        genero: "Crime, drama e ficção histórica",
        classificacao: "18",
    },

    {
        id: "Round 6",
        titulo: "Round 6",
        imagem: "Imagens/round6.jpg",
        descricao: "Centenas de jogadores falidos aceitam um estranho convite para um jogo de sobrevivência. Onde um prêmio milionário aguarda.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "3 Temporadas",
        genero: "Terror, comédia, ação, guerra, drama, sobrevivência, suspense e romance",
        classificacao: "16",
    },

    {
        id: "Avatar A lenda de Aang",
        titulo: "Avatar A lenda de Aang",
        imagem: "Imagens/avatar.jpg",
        descricao: "Avatar: A Lenda de Aang acompanha Aang, um menino de 12 anos que acorda após cem anos congelado em um iceberg.",
        trailer: "ID_YOUTUBE",

        // ALTERAÇÃO 21/08 - INFORMAÇÕES COMPLEMENTARES

        duracao: "3 Temporadas",
        genero: "Animação, ação, comédia, ficção científica e comédia",
        classificacao: "12",
    },

    {
        id: "the-last-of-us",
        titulo: "The Last of Us",
        imagem: "Imagens/thelastofus.jpg",
        descricao: "Joel e Ellie atravessam os Estados Unidos devastados por uma infecção enquanto desenvolvem uma forte ligação.",
        trailer: "ID_YOUTUBE",
        duracao: "2 Temporadas",
        genero: "Drama, ação e ficção científica",
        classificacao: "16",
    },  

    {
        id: "the-witcher",
        titulo: "The Witcher",
        imagem: "Imagens/thewitcher.jpg",
        descricao: "Geralt de Rívia, um caçador de monstros, percorre um mundo perigoso enquanto seu destino se conecta ao de uma princesa.",
        trailer: "ID_YOUTUBE",
        duracao: "4 Temporadas",
        genero: "Fantasia, ação e drama",
        classificacao: "16",
    },

    {
        id: "dark",
        titulo: "Dark",
        imagem: "Imagens/dark.jpg",
        descricao: "O desaparecimento de uma criança revela segredos familiares e uma misteriosa conexão entre diferentes períodos de tempo.",
        trailer: "ID_YOUTUBE",
        duracao: "3 Temporadas",
        genero: "Ficção científica, drama e suspense",
        classificacao: "16",
    },

    {
        id: "wandinha",
        titulo: "Wandinha",
        imagem: "Imagens/wandinha.jpg",
        descricao: "Wandinha Addams investiga acontecimentos sobrenaturais enquanto estuda em uma escola para jovens com habilidades especiais.",
        trailer: "ID_YOUTUBE",
        duracao: "2 Temporadas",
        genero: "Comédia, fantasia e suspense",
        classificacao: "14",
    },

    {
        id: "the-boys",
        titulo: "The Boys",
        imagem: "Imagens/theboys.jpg",
        descricao: "Um grupo de pessoas comuns enfrenta super-heróis corrompidos que abusam de seus poderes.",
        trailer: "ID_YOUTUBE",
        duracao: "5 Temporadas",
        genero: "Ação, drama e sátira",
        classificacao: "18",
    },

    {
        id: "cobra-kai",
        titulo: "Cobra Kai",
        imagem: "Imagens/cobrakai.jpg",
        descricao: "Anos após os eventos de Karatê Kid, Johnny Lawrence reabre o dojo Cobra Kai e reacende uma antiga rivalidade.",
        trailer: "ID_YOUTUBE",
        duracao: "6 Temporadas",
        genero: "Ação, comédia e drama",
        classificacao: "12",
    },

    {
        id: "arcane",
        titulo: "Arcane",
        imagem: "Imagens/arcane.jpg",
        descricao: "Duas irmãs ficam em lados opostos de um conflito entre as cidades de Piltover e Zaun.",
        trailer: "ID_YOUTUBE",
        duracao: "2 Temporadas",
        genero: "Animação, ação e fantasia",
        classificacao: "14",
    },

    {
        id: "one-piece",
        titulo: "One Piece",
        imagem: "Imagens/onepiece.jpg",
        descricao: "Monkey D. Luffy parte em uma grande aventura pelo mar para encontrar o lendário tesouro One Piece.",
        trailer: "ID_YOUTUBE",
        duracao: "2 Temporadas",
        genero: "Ação, aventura e fantasia",
        classificacao: "14",
    },

    {
        id: "the-office",
        titulo: "The Office",
        imagem: "Imagens/theoffice.jpg",
        descricao: "A rotina dos funcionários de uma empresa de papel é acompanhada com muito humor e situações inusitadas.",
        trailer: "ID_YOUTUBE",
        duracao: "9 Temporadas",
        genero: "Comédia",
        classificacao: "12",
    },

    {
        id: "friends",
        titulo: "Friends",
        imagem: "Imagens/friends.jpg",
        descricao: "Seis amigos enfrentam relacionamentos, problemas profissionais e situações divertidas enquanto vivem em Nova York.",
        trailer: "ID_YOUTUBE",
        duracao: "10 Temporadas",
        genero: "Comédia e romance",
        classificacao: "12",
    },

    {
        id: "loki",
        titulo: "Loki",
        imagem: "Imagens/loki.jpg",
        descricao: "Após roubar o Tesseract, Loki é levado para uma organização que controla as linhas do tempo do universo.",
        trailer: "ID_YOUTUBE",
        duracao: "2 Temporadas",
        genero: "Ação, aventura e ficção científica",
        classificacao: "12",
    },

    {
        id: "the-mandalorian",
        titulo: "The Mandalorian",
        imagem: "Imagens/themandalorian.jpg",
        descricao: "Um caçador de recompensas mandaloriano protege uma criança misteriosa enquanto atravessa a galáxia.",
        trailer: "ID_YOUTUBE",
        duracao: "3 Temporadas",
        genero: "Ação, aventura e ficção científica",
        classificacao: "12",
    },

    {
        id: "umbrella-academy",
        titulo: "The Umbrella Academy",
        imagem: "Imagens/umbrellaacademy.jpg",
        descricao: "Irmãos com superpoderes se reúnem após a morte do pai e tentam impedir uma ameaça ao fim do mundo.",
        trailer: "ID_YOUTUBE",
        duracao: "4 Temporadas",
        genero: "Ação, fantasia e drama",
        classificacao: "16",
    },

    {
        id: "black-mirror",
        titulo: "Black Mirror",
        imagem: "Imagens/blackmirror.jpg",
        descricao: "A série apresenta histórias independentes que exploram os impactos da tecnologia na sociedade e na vida humana.",
        trailer: "ID_YOUTUBE",
        duracao: "7 Temporadas",
        genero: "Ficção científica, drama e suspense",
        classificacao: "16",
    },

    {
        id: "lost",
        titulo: "Lost",
        imagem: "Imagens/lost.jpg",
        descricao: "Sobreviventes de um acidente aéreo ficam presos em uma ilha cheia de mistérios e acontecimentos inexplicáveis.",
        trailer: "ID_YOUTUBE",
        duracao: "6 Temporadas",
        genero: "Drama, aventura e suspense",
        classificacao: "14",
    },

    {
        id: "the-walking-dead",
        titulo: "The Walking Dead",
        imagem: "Imagens/thewalkingdead.jpg",
        descricao: "Um grupo de sobreviventes tenta continuar vivo em um mundo dominado por mortos-vivos.",
        trailer: "ID_YOUTUBE",
        duracao: "11 Temporadas",
        genero: "Terror, drama e ação",
        classificacao: "16",
    },
];